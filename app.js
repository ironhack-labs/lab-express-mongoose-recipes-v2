...
app.post("/recipes", (req, res) => {
  Recipe.create(req.body)
    .then((createdRecipe) => {
      console.log("This is the created recipe:", createdRecipe);
...
app.get("/recipes", (req, res) => {
  Recipe.find()
    ...
    .catch((err) => {
      res.status(500).json({ message: "Error retrieving all recipes" });
    });
});
...
app.put("/recipes/:id", (req, res) => {
  const recipeID = req.params.id;
  Recipe.findByIdAndUpdate(recipeID, req.body)
    .then((updatedRecipe) => {
